...
ppm <- 35
ms1_intensity_cutoff <- 5000	          #approx. 0.01%
...
mzml_files_pos <- mzml_files_pos[grep("R-", mzml_files_pos, invert=FALSE)]
# Unfortunately, R-wanstorfii-1 is mis-identification of R-glauca !
mzml_files_pos <- mzml_files_pos[grep("R-warnstorfii-1", mzml_files_pos, invert=TRUE)]
mzml_files_pos <- mzml_files_pos[grep("R-bifurca", mzml_files_pos, invert=TRUE)]
...
mzml_pheno_pos <- data.frame(sample_name=mzml_names_pos, sample_group=gsub(pattern="\\.\\d.*", replacement="", x=mzml_names_pos, perl=TRUE))
mzml_pheno_samples_pos <- as.factor(mzml_pheno_pos$sample_group)
mzml_pheno_colors_pos <- c("goldenrod4","skyblue4","forestgreen")
mzml_pheno_colors_samples_pos <- c("goldenrod4","goldenrod4","goldenrod4","skyblue4","skyblue4","skyblue4","forestgreen","forestgreen","forestgreen")
...
if (polarity=="negative") {
	...
	ms1_data_pos <- groupChromPeaks(ms1_data_pos, param=PeakDensityParam(sampleGroups=ms1_data_pos$sample_group, minFraction=0.7, bw=0.25))
}
...
if (SIRIUS_VERSION == 4) {
	...
	classifier_canopus_pos <- read.table(file=paste0("data/pos_ms2_canopus_summary.tsv"), header=TRUE, sep="\t", quote="\"", comment.char="", fill=TRUE, dec=".", stringsAsFactors=FALSE)#, encoding="UTF-8", fileEncoding="UTF-8")
...
# ---------- Variable Selection ----------
# Random Forest
sel_rf_pos <- f.select_features_random_forest(feat_matrix=feat_list_pos, sel_factor=as.factor(mzml_pheno_samples_pos), sel_colors=mzml_pheno_colors_pos, tune_length=10, quantile_threshold=0.95, plot_roc_filename="plots/pos_ms1_select_rf_roc.pdf")
...
# PLS
sel_pls_pos <- f.select_features_pls(feat_matrix=feat_list_pos, sel_factor=as.factor(mzml_pheno_samples_pos), sel_colors=mzml_pheno_colors_pos, components=2, tune_length=10, quantile_threshold=0.997, plot_roc_filename="plots/pos_ms1_select_pls_roc.pdf")
...
# PLS
sel_pls_pos <- f.select_features_pls(feat_matrix=class_list_pos, sel_factor=mzml_pheno_samples_pos, sel_colors=mzml_pheno_colors_pos, components=2, tune_length=10, quantile_threshold=0.95, plot_roc_filename="plots/pos_ms2_classes_select_pls_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_pls_pos$`_selected_variables_`)))
