...
mzml_files_neg <- mzml_files_neg[grep(x=mzml_files_neg, pattern=paste0(radula_environmental_factors$Sample.name,collapse="-|"), invert=FALSE)]
...
mzml_pheno_neg <- data.frame(sample_name=gsub(pattern="\\.auto.MSMS.neg_..*", replacement="", x=mzml_names_neg, perl=TRUE), 
							 sample_group=radula_environmental_factors$Location)

mzml_pheno_samples_neg <- as.factor(mzml_pheno_neg$sample_group)
mzml_pheno_colors_neg <- data.frame(cols=c("dodgerblue3", "darkgoldenrod3", "mediumorchid4"))
...
par(mfrow=c(1,1), mar=c(4,4,4,1), oma=c(0,0,0,0), cex.axis=0.9, cex=0.6)
plot(chromatogram(raw_data_neg, mz=c(213, 216), rt=c(214, 220)), col=mzml_pheno_colors_samples_neg, main = "EIC of Kinetin")
plot(chromatogram(raw_data_neg, mz=c(282, 285), rt=c(545, 555)), col=mzml_pheno_colors_samples_neg, main = "EIC of Biochanin A")
plot(chromatogram(raw_data_neg, mz=c(244, 247), rt=c(337, 346)), col=mzml_pheno_colors_samples_neg, main = "EIC of N-(3-Indolylacetyl)-L-Ala")

plot(chromatogram(raw_data_neg, mz=c(278, 281), rt=c(667, 680)), col=mzml_pheno_colors_samples_neg, main = "EIC of Radulanin A")
...
plot(chromatogram(raw_data_neg, mz=c(280, 283), rt=c(613, 624)), col=mzml_pheno_colors_samples_neg, main = "EIC of 4-prenyldihydropinosylvin")
plot(chromatogram(raw_data_neg, mz=c(348, 351), rt=c(750, 765)), col=mzml_pheno_colors_samples_neg, main = "EIC of Bibenzyl-CBG")
plot(chromatogram(raw_data_neg, mz=c(338, 341), rt=c(743, 753)), col=mzml_pheno_colors_samples_neg, main = "EIC of Bibenzyl 1")
dev.off()
...
if (polarity=="negative") {
	...
	ms1_params_neg <- CentWaveParam(ppm=9.5, mzCenterFun="wMean", peakwidth=c(4, 33), prefilter=c(2, 140), mzdiff=0.0012, snthresh=10, noise=0, integrate=1,
									firstBaselineCheck=TRUE, verboseColumns=FALSE, fitgauss=FALSE, roiList=list(), roiScales=numeric())
}
...
feat_list_neg[is.na(feat_list_neg)] <- 1
feat_list_neg[which(feat_list_neg < 0)] <- 1
feat_list_neg[is.infinite(feat_list_neg)] <- 1
...
pdf(file="plots/neg_ms1_feature_table_pca_23.pdf", encoding="ISOLatin1", pointsize=10, width=6, height=4, family="Helvetica")
...
model_varpart_neg <- varpart(scale(feat_list_neg), ~ radula_environmental_factors$Location, ~ radula_environmental_factors$Host.tree, radula_environmental_factors$Biological.Replicate)
plot(model_varpart_neg, Xnames=c("Location","Host tree","Biological Replicate"), cutoff=0, cex=1.2, id.size=1.2, digits=1, bg=c("blue","green","red"))

model_varpart_neg <- varpart(scale(feat_list_neg), ~ radula_environmental_factors$Location, ~ radula_environmental_factors$East, radula_environmental_factors$North)
plot(model_varpart_neg, Xnames=c("Location","Eastern Longitude","Northern Latitude"), cutoff=0, cex=1.2, id.size=1.2, digits=1, bg=c("blue","green","red"))

model_varpart_neg <- varpart(scale(feat_list_neg), ~ radula_environmental_factors$Location, ~ radula_environmental_factors$Host.tree.family, as.numeric(as.factor(radula_environmental_factors$Collection.Month)))
plot(model_varpart_neg, Xnames=c("Location","Host tree family","Collection month"), cutoff=0, cex=1.2, id.size=1.2, digits=1, bg=c("blue","green","red"))
dev.off()
...
# ==================== Host tree ====================
# ---------- Diversity ----------
mzml_pheno_samples_neg <- radula_environmental_factors$Host.tree
mzml_pheno_colors_neg <- data.frame(cols=2:(length(unique(mzml_pheno_samples_neg))+1))
rownames(mzml_pheno_colors_neg) <- unique(mzml_pheno_samples_neg)
mzml_pheno_colors_samples_neg <- sapply(mzml_pheno_samples_neg, function(x) { x <- mzml_pheno_colors_neg[,1][which(x==unique(mzml_pheno_neg$sample_group))] } )

# Plot unique features
pdf(paste("plots/neg_ms1_host_tree_richness_unique.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
...
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=unique(mzml_pheno_samples_neg), xpd=TRUE, cex=0.9)
...
# Plot total features
pdf(paste("plots/neg_ms1_host_tree_richness_total.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
...
# Plot Shannon index
pdf(paste("plots/neg_ms1_host_tree_diversity_shannon.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
...
pdf(paste("plots/neg_ms1_host_tree_diversity_pielou.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
...
# ---------- Variable Selection ----------
# Random Forest
sel_rf_neg <- f.select_features_random_forest(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, tune_length=10, quantile_threshold=0.99, plot_roc_filename="plots/neg_ms1_host_tree_select_rf_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_rf_neg$`_selected_variables_`)))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_rf_neg$`_selected_variables_`, filename="plots/neg_ms1_host_tree_select_rf.pdf", main="Random Forest")

# BORUTA
#library(doParallel)
#cl <- makeCluster(24)
#registerDoParallel(cl)
#sel_boruta_rf <- f.select_features_random_forest_boruta(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, sel_method="best", tune_length=2, pvalue=0.01, mcAdj=TRUE, maxRuns=10000, plot_roc_filename="plots/neg_ms1_select_host_tree_boruta_roc.pdf")
#print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_boruta_rf[["_selected_variables_"]])))
#f.heatmap.selected_features(feat_list=feat_list, sel_feat=sel_boruta_rf, filename="plots/neg_ms1_host_tree_select_boruta.pdf", main="BORUTA")
#stopCluster(cl)

# PLS
sel_pls_neg <- f.select_features_pls(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, components=length(unique(mzml_pheno_samples_neg)) - 1, tune_length=10, quantile_threshold=0.99, plot_roc_filename="plots/neg_ms1_host_tree_select_pls_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_pls_neg$`_selected_variables_`)))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_pls_neg$`_selected_variables_`, filename="plots/neg_ms1_host_tree_select_pls.pdf", main="PLS")
...
sel_splsda_neg <- f.select_features_splsda(feat_matrix=feat_list_neg, sel_colors=mzml_pheno_colors_neg$cols, sel_factor=as.factor(mzml_pheno_samples_neg), tune_components=length(unique(mzml_pheno_samples_neg)), sel_components=c(1:(length(unique(mzml_pheno_samples_neg)) - 1)), folds_number=10, keepx=model_splsda_keepx_neg, plot_roc_filename="plots/neg_ms1_host_tree_select_splsda_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_splsda_neg$'_selected_variables_')))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_splsda_neg$'_selected_variables_', filename="plots/neg_ms1_host_tree_select_splsda.pdf", main="sPLS-DA")
...
# Plot unique features
pdf(paste("plots/neg_ms1_location_richness_unique.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
boxplot(model_div_neg$unique ~ mzml_pheno_samples_neg, col=mzml_pheno_colors_neg$cols, names=NA, main="Number of unique features", xlab="treatment", ylab="number of unique features")
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=unique(mzml_pheno_samples_neg), xpd=TRUE, cex=0.9)
div_tukey <- tukey.test(response=model_div_neg$unique, term=as.factor(mzml_pheno_samples_neg))
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[4]+(par("usr")[4]-par("usr")[3])/40, adj=0.5, labels=div_tukey[,1], xpd=TRUE, cex=0.8)
...
# Plot total features
pdf(paste("plots/neg_ms1_location_richness_total.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
boxplot(model_div_neg$features ~ mzml_pheno_samples_neg, col=mzml_pheno_colors_neg$cols, names=NA, main="Number of features", xlab="treatment", ylab="number of features")
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=unique(mzml_pheno_samples_neg), xpd=TRUE, cex=0.9)
div_tukey <- tukey.test(response=model_div_neg$features, term=as.factor(mzml_pheno_samples_neg))
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[4]+(par("usr")[4]-par("usr")[3])/40, adj=0.5, labels=div_tukey[,1], xpd=TRUE, cex=0.8)
...
# Plot Shannon index
pdf(paste("plots/neg_ms1_location_diversity_shannon.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
boxplot(model_div_neg$shannon ~ mzml_pheno_samples_neg, col=mzml_pheno_colors_neg$cols, names=NA, main="Shannon diversity (H\')", xlab="treatment", ylab="Shannon diversity index (H\')")
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=unique(mzml_pheno_samples_neg), xpd=TRUE, cex=0.9)
div_tukey <- tukey.test(response=model_div_neg$shannon, term=as.factor(mzml_pheno_samples_neg))
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[4]+(par("usr")[4]-par("usr")[3])/40, adj=0.5, labels=div_tukey[,1], xpd=TRUE, cex=0.8)
...
# Plot Pielou evenness for species
pdf(paste("plots/neg_ms1_location_diversity_pielou.pdf",sep=""), encoding="ISOLatin1", pointsize=10, width=5, height=5, family="Helvetica")
boxplot(model_div_neg$pielou ~ mzml_pheno_samples_neg, col=mzml_pheno_colors_neg$cols, names=NA, main="Pielou\'s evenness", xlab="treatment", ylab="Pielou diversity index (J)")
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=unique(mzml_pheno_samples_neg), xpd=TRUE, cex=0.9)
div_tukey <- tukey.test(response=model_div_neg$pielou, term=as.factor(mzml_pheno_samples_neg))
text(1:length(unique(mzml_pheno_samples_neg)), par("usr")[4]+(par("usr")[4]-par("usr")[3])/40, adj=0.5, labels=div_tukey[,1], xpd=TRUE, cex=0.8)
...
# ---------- Variable Selection ----------
# Random Forest
sel_rf_neg <- f.select_features_random_forest(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, tune_length=10, quantile_threshold=0.99, plot_roc_filename="plots/neg_ms1_location_select_rf_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_rf_neg$`_selected_variables_`)))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_rf_neg$`_selected_variables_`, filename="plots/neg_ms1_location_select_rf.pdf", main="Random Forest")

# BORUTA
#library(doParallel)
#cl <- makeCluster(24)
#registerDoParallel(cl)
#sel_boruta_rf <- f.select_features_random_forest_boruta(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, sel_method="best", tune_length=2, pvalue=0.01, mcAdj=TRUE, maxRuns=10000, plot_roc_filename="plots/neg_ms1_select_location_boruta_roc.pdf")
#print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_boruta_rf[["_selected_variables_"]])))
#f.heatmap.selected_features(feat_list=feat_list, sel_feat=sel_boruta_rf, filename="plots/neg_ms1_location_select_boruta.pdf", main="BORUTA")
#stopCluster(cl)

# PLS
sel_pls_neg <- f.select_features_pls(feat_matrix=feat_list_neg, sel_factor=as.factor(mzml_pheno_samples_neg), sel_colors=mzml_pheno_colors_neg$cols, components=length(unique(mzml_pheno_samples_neg)) - 1, tune_length=10, quantile_threshold=0.99, plot_roc_filename="plots/neg_ms1_location_select_pls_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_pls_neg$`_selected_variables_`)))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_pls_neg$`_selected_variables_`, filename="plots/neg_ms1_location_select_pls.pdf", main="PLS")
...
sel_splsda_neg <- f.select_features_splsda(feat_matrix=feat_list_neg, sel_colors=mzml_pheno_colors_neg$cols, sel_factor=as.factor(mzml_pheno_samples_neg), tune_components=length(unique(mzml_pheno_samples_neg)), sel_components=c(1:(length(unique(mzml_pheno_samples_neg)) - 1)), folds_number=10, keepx=model_splsda_keepx_neg, plot_roc_filename="plots/neg_ms1_location_select_splsda_roc.pdf")
print(paste("Number of selected features:", f.count.selected_features(sel_feat=sel_splsda_neg$'_selected_variables_')))
f.heatmap.selected_features(feat_list=feat_list_neg, sel_feat=sel_splsda_neg$'_selected_variables_', filename="plots/neg_ms1_location_select_splsda.pdf", main="sPLS-DA")
